...
| `phasor_isolate` | EventHandler | `image_in`, `completion_in`, `control_in` | `result_out`, `call_out`, `diagnostic`, `exit` | Realm, machine, objects, heap, collection, jobs, promises, pending calls, module evaluation. The `grants` parameter states the capabilities this deployment admits, one binding each after the general `host` call; resources providers opened for the task are held here, so a granted member answering with a handle has somewhere for it to live |
| `phasor_host_router` | Protocol | `call_in`, `reply_in` | `completion_out`, `request_out` | Binding admission, routing, correlation, bounded in-flight table. A binding is an index the isolate admitted; an adapter knows its operations by method number, and this is where one becomes the other. The `interface` parameter names what the adapter behind it serves and `grants` states the same list the isolate was given, in the same order; a call on any other interface is refused here, because nothing behind it could answer |
...
| `phasor_store` | Adapter | `request_in` | `reply_out` | An object store: keys and values a program put there and nothing else. Holds no bytes itself — one granted call becomes one operation on Fluxor's `storage.object` and `storage.namespace`, under the key prefix its `namespace` parameter names. Answers with bytes behind the frame, and with handles for `open`, under a call `quota` |
| `phasor_fs` | Adapter | `request_in` | `reply_out` | Files under the directory the graph runs in, and nothing above it: an absolute name or one that climbs is refused before anything is opened. Reading and writing are separate bindings, and `writable` gates writing in the adapter itself, which is what lets a read-only target serve the read members alone. Declares the Fluxor `fs` contract in its manifest |
| `phasor_net` | Adapter | `request_in`, `net_in` | `reply_out`, `net_out` | A TCP connection to the authority `connect` names, `host:port`, admitted by the `origins` allow-list and written into the connect record for the stack to resolve; `connect()` naming nothing goes to the graph's `authority`. Answers with a handle, carries bytes both ways behind the frame, and holds a read until the connection has some. Speaks Fluxor's stream protocol, to the socket or to `tls`, under a call `quota` |
| `phasor_http` | Adapter | `request_in`, `reply_in`, `body_in` | `reply_out`, `publish_out`, `plain_out` | One request against the authority `send` names, admitted by the `origins` allow-list and written into the exchange record for the open provider behind either leg to dial; the scheme picks the leg, `https` to `publish_out` and `http` to `plain_out`. Answered with a handle over the response: the status, the header block, and the body read through that handle. The protocol is the provider's — framing, chunked decoding, and a connection's lifetime never reach this module or the program. Under a call `quota` |
| `phasor_ws` | Adapter | `request_in`, `event_in`, `frames_in` | `reply_out`, `open_out`, `frames_out` | A WebSocket over the origin its `authority` parameter names: `open` takes the resource and not the host. The upgrade, the accept it verifies, the masking and the frame codec are the provider's; messages cross as `WsFrame`, so which link a message arrived on and whether it was text or binary are read from the contract |
...
| `phasor_shell` | Cli | `args`, `stdin`, `surface_in`, `clock_reply`, `entropy_reply`, `store_reply`, `fs_reply`, `net_reply`, `http_reply`, `ws_reply` | `stdout`, `exit`, `clock_call`, `entropy_call`, `store_call`, `fs_call`, `net_call`, `http_call`, `ws_call` | The shell: a script, `-e`, or a REPL over one realm, each input a bounded task. It reads the standard surface from `surface_in` and compiles it ahead of the program, or leaves it out under `--bare`. A `--grant` admits one of the interfaces its graph wires to the adapters, each as a namespace of members, and `--steps` sets the fuel. Installed as the `phasor` applet from `packaging/cli/` |
...
| `phasor_expect` | States a program as `source` and the answer it must produce as `expect` (or a `shape` such as `digits`); feeds the one to `phasor_compile` and completes only when the isolate's result was the other, which is how a self-contained WebAssembly bundle holds a capability round trip without a standard input or output. When the program is a module, its image comes back on `image_in` and leaves on `record_out` as the one linker record `phasor_link` needs, under the `specifier` parameter's name | Fixture only |
...
| `phasor_store` | entries held open at once, bytes per key and per value across the seam, calls answered |
...
| `phasor_http` | requests in flight, bytes of head and of body staged, calls answered |
| `phasor_ws` | links open at once, bytes per message, frames staged per output, calls answered |
