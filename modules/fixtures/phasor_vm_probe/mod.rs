...
fn run_chain(
    ...
    let image_length = {
        ...
        // SAFETY: the staged source is read only while the front end writes
        // elsewhere in the storage.
    ...
    if stepwise {
        // SAFETY: the image is staged and is not written again while it runs,
        // so the machine may keep reading it while the rest of the storage moves.
